...
- Дальнейшая работа строится на основе какого протокола будет происходить обмен данными:
	- "tcp_client_task()" создание и запуск задачи соединения socket/tcp_client в неблокирующем режиме.
    Файл "tcp_client_v4.c". (использовать при локальном подключении через WiFi).
	- "websocket_client_task()" создание и запуск задачи соединения websocket через TLS.
    Файл "app_websocket.c"(использовать при подключении своего разработанного сервера, требует доработки).
...
5 Инициализация интерфейса CAN через запуск функции "init_CAN(CAN_TX,CAN_RX)", файл CAN.c. Передача семафора,
...
- keepalive = 30 сек. максимальное время, в течение которого клиент или брокер могут не отправлять
  управляющий пакет;
- clean_session = true. Чистый сеанс (true):
	Когда клиент подключается, брокер удаляет всю информацию о предыдущих сеансах для этого ClientID.
	Когда клиент отключается, все его подписки удаляются, а все сообщения QoS 1 или QoS 2, которые были
	поставлены в очередь для него (если он был временно недоступен), отбрасываются брокером.
	Это как начинать с чистого листа при каждом подключении так как при работе в автономном режиме могут
	измениться настройки системы;
...
2 При установлении соединения (новый сеанс) первым посылается сообщение: topic = "status", msg = "on",
  означающее что система подключилась.
	Затем отсылается информация по каждому устройству сети CAN: topic = "inf/№", см. "Протокол обмена данными
	по MQTT".
