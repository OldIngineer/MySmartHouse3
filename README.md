...
включается светодиод "WORK". В случае неудачи создания соединения по протоколу TCP светодиод "WORK" выключается.. Затем запускается mqtt client через соединение websocket security, в случае неудачного соединения загорается светодиод "ALARM", который потухнет при создании соединения с сервером. Одновременно создается задача инициализации sms модема.    
По кратковременному нажатию кнопки "INIT" устройства "master" происходит опрос подключенных в сеть CAN 
устройств "slave" в режиме разделения во времени, для повышения достоверности сканирование сети происходит
несколько раз при суммировании адресов ответивших устройств. По результату опроса составляется список
обнаруженных устройств. Затем у этих устройств последовательно запрашиваются все СЕРВИСЫ ПРОФИЛЯ.
Сервисы передаются в пакетном режиме: каждая характеристика сервиса это отдельный кадр данных CAN.
Полученная информация заносится во флеш-память устройства "master", откуда может быть востребована 
дальнейшей работы сети или по команде внешнего устройства (сервер, сотовый телефон), кроме того если
установлено соединение с брокером MQTT то вся информация по сервисам подключенных в сети устройств 
передается брокеру в виде топиков.
В случае соединения сети TWAI с брокером mqtt информация сервисов типа функциональности всех устройств
передается ему в виде топиков, для обновления данных которые могли изменится при автономной работе.
При срабатывании датчика устройство выставляет в сеть СОБЫТИЕ (кадр данных с информацией об
изменении параметра) и для надежности производятся повторы публикации. Исполнительные устройства сети,
подписанные на данное СОБЫТИЕ с помощью сценариев, также в качестве подтверждения выполненных действий
выставляют свои СОБЫТИЯ. Все СОБЫТИЯ в сети регистрируются устройством "master" во флеш-памяти и 
отправляются брокеру MQTT в виде топиков.
ПРИМЕЧАНИЕ - 
1) Все запросы данных в сети CAN повторяются несколько раз при отсутствии ответа.
2) Все данные передаваемые в пакетном режиме содержат номер кадра и проверяются приемником на
   соответствие в порядке очередности.
3) При потере хотя бы одного кадра все данные запрашиваются повторно, возможно несколько раз.
4) При опубликовании "события" подчиненным устройством согласно сценария всегда производятся
    повторы оговоренное число раз.
       
	В каждом устройстве создана область nvs-памяти для хранения 63 профилей возможных членов сети.
 Изначально в каждое устройство при программировании заносится информационный сервис в область
nvs-памяти "profile" в пространстве имен "device_inf", который затем при первом запуске переписывается
 в область памяти соответствующей локальному номеру присвоенному данному устройству. Кроме информационного
 сервиса там же хранятся сервисы функциональности, расположенные в пространстве имен типа "type*",
 где * - это номер функциональности в списке перечесления кодов информационного сервиса.
	Для оперативной работы программы (чтобы каждый раз не обращаться к nvs-памяти) используются
...
- таблица ячейки которой содержат код типа функциональности для 63 устройств сети по 9 типам,
   включая информационный, в порядке перечисления их в информационном сервисе устройства.
...
- группа программ "TYPE_*.c", где * означает код функциональности "TYPE_*.c" собранные в одном месте функции относящиеся только к данному типу функциональности (драйвера и прочее...). Например: "TYPE_070.c" содержит драйвер последовательного однопроводного интерфейса, данные с точностью до 0,1*С/% (по 2 байта). Темп получения показаний 20 сек. Если изменение показания температуры от запомненного более 1*С, производится запись в память и выдача топиков в MQTT с точностью до 1*С/% и значение температуры запоминается для сравнения. Также организована корректировка показаний датчика через запись констант при программировании в область памяти "user_nvs".    
...
- "app_mqtt_wss.c" драйвер защищенного соединения сети TWAI с брокером MQTT через соединение WEBSocket
   использует протокол шифрования TLS с проверкой сертификата сервера.    
- "sms.c" содержит задачу по обмену sms через модуль GSM. Этот вариант обмена данными являеися резервным на случай отсутствия интернета.
...
- get_DeviceInf();//из nvs памяти считывается информация информационного сервиса: локальный номер,
  ...
  данной функциональности)путем запуска функций "init_type_*", которые находятся в одноименных файлах    
  "TYPE_*.c"
...
- запуск функции "init_sms_modem()" из файла sms.c. Создается задача приема/передачи данных через sms (в отсутствии интернета).        
...
- "mqtt_wss_client_task()" создание и запуск задачи соединения клиента с брокером MQTT через wss
    ...
    качества обслуживания QoC 1 (подтвержденная доставка) с запоминанием сообщения до замены его новым по тому же топику. По умолчанию размер буферов приема/передачи установлен в файле sdkconfig - 1024 байт(по умолчанию).      
...
ПРИМЕЧАНИЕ
1 Перед первой компиляцией необходимо задать целевое устройство: idf.py set-target esp32c6    
2 В начале необходимо записать в память профиль устройства, а затем программу.
...
Смещение раздела прошивки должно быть выровнено на 64 КБ, начальные 36 КБ (9 секторов)
зарезервированы для загрузчика и таблицы разделов.
...
Откройте в главной директории проекта файл profile.csv с помощью редактора "блокнот",
а если его нет создайте в соответствии с требованиями изложенными в 
...
d_inf,namespace,,    
#номер устройства в локальной сети CAN    
name1,data,string,"01"    
#серийный номер устройства    		
name2,data,string,"00001"    
#Подчиненность    
name3,data,string,"master"    
#наименование контроллера    
name4,data,string,"device_one"    
#присвоенное имя пользователем    
name5,data,string,"no_name"    
#версия программы    
name6,data,string,"v1"    
#перечисление функциональных типов    
name7,data,string,"070 210 0A0 220#"    
...
Затем записывается утилитой esptool.py в NVS память микропроцессора. Например команда записи
через порт 	вывода "com8" во "flash память" с адреса "0x5FE000" файла "profile.bin":
...
если произошел сбой невозможно по сбросу "RESET" запустить монитор с начальной точки программы).    
		АНАЛОГИЧНО СОЗДАЕТСЯ ОБЛАСТЬ КОНСТАНТ
	Создается файл user_nvs.csv в главной директории проекта с помощью редактора "блокнот", куда заносятся констанды коррекции датчиков устройства.

	#КОНСТАНТЫ УСТРОЙСТВА
	key,type,encoding,value
	const,namespace,,
	#===ДАТЧИК DTH22====
	#поправка на температуру, 0,1*C
	corec_t,data,string,"-34"
	#поправка на влажность, 0,1%				
	corec_h,data,string,"-2"
	#============================

Созданный/измененный файл превращается в бинарный файл с помощью команды: "python nvs_partition_gen.py generate user_nvs.csv user_nvs.bin 0x6000".    
Затем записывается программой "flash_download_tool" в NVS память микропроцессора с адреса 0х604000, согласно таблице разбивки памяти.
...
3 При написании программы обмена данными по беспроводному интерфейсу wss (websocket security)за основу взят 
	пример: https://circuitlabs.net/websocket-client-implementation/
	
4 При написании программы клиента MQTT с подключением через протокол WebSocket Secure использовалось описание
	ESP-IDF:
...
5 При написании программы драйвера датчика температуры/влажности использовалась программа описанная в: https://habr.com/ru/articles/989832/ 
