...
void mqtt_msg_in(int topic_len, char *topic, int data_len, char *data)
{
   //Если топик является командой (начинается с "com")
    ...
    //если команда для этого устройства            
      if(htol(num)==htol(number)) {
        //если указанный тип присутствует в перечне типов этого устройства
    ...
    //если команда не для этого устройства, а для подчиненного
      if(strcmp(num, number) != 0) {
        //если указанный локальный номер контроллера присутствует в перечне
        uint8_t num_ok = 0;
      ...
      if(num_ok != 0) {
        ...
        //JSON разбор   {”event”:{“2/A0/p/2”:”1”},”make”:{“1”:”1”}}
          ...
          //printf("num_e:%s, type_e:%s, theme_e:%s, ordinal_e:%s, value_e:%s, ordinal_m:%s, value_m:%s\n", num_e, type_e, theme_e, ordinal_e, value_e, ordinal_m, value_m );
        //формирование кадров данных для посылки в CAN
          uint8_t num_cadr = (htol(ordinal)*2+63);//номер 1 кадра
          ...
          uint64_t cod_event = (id_even<<24) + (htol(ordinal_e)<<16) + htol(value_e);
