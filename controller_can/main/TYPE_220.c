...
void init_type_220(uint8_t num)
{
  ...
  //tab_param[num][0] = strtol("220", NULL, 16);
  tab_param[num][0] = 0x220;//код типа
  tabl_type[htol(number)][num] = 0x220;//запись типа функциональности
  char data[] = "220";
  //запись типа в память nvs
  change_profile_nvs(htol(number),0x220,'n',1,data,1);
  //запись признака окончания характеристик #
  char name_param[] = "#";
  change_profile_nvs(htol(number),0x220,'t',3,name_param,1);
}